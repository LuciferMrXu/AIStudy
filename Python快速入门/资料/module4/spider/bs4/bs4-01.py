...
# 请求页面获取html对象
# response = requests.get('https://m.weibo.cn/').content.decode('utf-8')
# print(response)

# # 使用 beautifulsoup 解析 请求到的html文档
# soup = BeautifulSoup(response,'lxml')   # lxml方式   解析速度快 容错率高
# print(soup)        # 解析html文档对象
# print(type(soup))

# soup = BeautifulSoup(response,'html.parser')   # html.parser方式
...
# 解析 web.html
result = BeautifulSoup(open('web.html',encoding='utf-8'),'html.parser')  # 打开本地的html文件
# print(result)

# # 四大对象之 Tag(标签)
# print(result.html.body.div.span)   # 默认会找第一个名字叫这个的Tag
# print(type(result.html.body.div.span))


# # 四大对象之 NavigableString (标签中的文本)
# print(result.html.body.div.span.a.string)  # 使用string必须保证里面没有子标签
# print(type(result.html.body.div.span.a.string))
...
# print(result.html.body.div.children)  # 只返回标签中的文本,返回的是可迭代对象
...
# print(result.html.body.div.next_sibling.next_sibling.next_sibling.next_sibling) # next_sibling把换行符也当作一个节点
...
# # find_all 获取所有的子节点
# print(result.find_all('span'))  # 传递字符串  括号里填想要找的标签名
...
html的三大选择器
   ...
   id选择器和class选择器就是相当于给标签取名字
   id的名称是不能重复的  class可以重复
'''
# print(result.find_all(id = 'foot'))
# print(result.find_all(class_ = 'one'))  # class 是属性,要加一个下划线
# print(result.find_all(name='aa'))       # name 表示标签名,会返回一个空列表
# print(result.find_all(class_ = 'one',attrs = {'name':'aa'})) # 通过class选择器名加上attrs属性名进行精确查找

# print(result.find_all(text = 'haha'))   # 查找文本



# # find 查找单个节点
# # 获取文本
# print(result.find(class_='right'))   # 首先进行定位
# print(result.find(class_ = 'right').find_all('a')[2:])   # 获取的是个列表,可进行切片选择
# print(result.find(class_ = 'right').find_all('a',limit = 2))   # limit也可限定个数获取
...
# for i in result.find(class_ = 'right').find_all('a')[2:]:  # 提取属性
    # print(i['href'])   # 第一种,获取属性链接
    # print(i.string)         # 获取文本
    # print(i.attrs['href'])  # 第二种获取属性链接的方法


# html定位方法
# print(result.find(id = 'content').contents[-2])    # 不推荐
# print(result.find(id = 'content').div.next_sibling.next_sibling.find_all('a'))  # 推荐使用
